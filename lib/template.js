...
var normalizelf = require('./util/normalizelf');
...
template.setDelimiters = function (name) {
    ...
    _.extend(_.templateSettings, delimiters.lodash);
...
template.process = function (tmpl, options) {
    ...
    try {
        ...
        while (tmpl.indexOf(delimiters.opener) >= 0) {
            tmpl = _.template(tmpl, options)(data);
        ...
        if (String(e) === 'SyntaxError: Unexpected token ILLEGAL' && /\n|\r/.test(tmpl)) {
            console.error('A special character was detected in this template. ' +
