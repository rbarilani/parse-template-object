...
parser.set = function (prop, value) {
    return util.namespace.set(this.data, this.getPropString(prop), value);
};

parser.getRaw = function (prop) {
    if (prop) {
        // Prop was passed, get that specific property's value.
        return util.namespace.get(this.data, this.getPropString(prop));
    } else {
...
parser.process = function (raw) {
    return util.recurse(raw, function (value) {
        ...
        if (matches) {
            ...
            var toExclude = this.options.excludeProps.find((key) => {
              return new RegExp(`^${key}$`).test(matches[1]);
            });

            if(toExclude) { return value; }
        ...
        return template.process(value, {data: this.data});
    }.bind(this));
};
